...
public class SubsCont extends Attributes {
    ...
    public String SubMyProgress(@PathVariable Long id) {
        ...
        if (purchased.getCount() >= purchased.getSub().getModules()) {
            ...
            Statics statics = purchased.getSub().getStatics();
            statics.setCountNow(statics.getCountNow() - 1);
            statics.setCountComplete(statics.getCountComplete() + 1);
            staticsRepo.save(statics);
        }
    ...
    public String SubBuy(@RequestParam Long teacherId, @PathVariable Long id) {
        ...
        sub.getStatics().setCountNow(sub.getStatics().getCountNow() + 1);
        subsRepo.save(sub);
